...
| `alias` | VARCHAR(50) | nullable | CLI에서 사용하는 프로젝트 별칭 (`.gpmrc`의 alias 키와 매핑) |
| `prefix` | TEXT | UNIQUE | 프로젝트 고유 prefix (자동 생성, 예: GPAM) |
| `status_options` | JSON | nullable | 프로젝트 Status 필드의 옵션 목록 (칸반 컬럼 순서 결정) |
| `status_options_fetched_at` | DATETIME | nullable | status_options를 마지막으로 가져온 시각 |
...
CREATE TABLE projects (
    ...
    prefix TEXT UNIQUE,
    status_options JSON,
    status_options_fetched_at DATETIME,
...
- `alias`는 CLI `.gpmrc`의 프로젝트 alias와 연동되며, 웹 UI에서 탭 레이블로 사용 (null이면 `Project #N` 형식으로 표시)
- `status_options`는 GitHub Project의 Status 필드 옵션을 캐싱하여 칸반 보드 컬럼 순서를 결정
...
하나의 `.gpmrc`에 여러 GitHub Project를 등록하고 alias로 구분합니다.
...
| `defaultProject` | string | O | 기본 프로젝트 alias (`projects` 키 중 하나) |
| `projects` | object | O | alias → ProjectConfig 매핑 |
...
이전 버전의 단일 프로젝트 `.gpmrc` 형식도 지원합니다. CLI가 로드 시 자동으로 멀티 프로젝트 형식으로 정규화합니다.
...
정규화 로직:
- `projectNumber`가 존재하고 `projects`가 없으면 레거시 형식으로 판단
- `projects: { "default": { projectNumber, projectUrl } }`, `defaultProject: "default"`로 변환
- 변환 결과는 메모리에서만 사용하며 파일을 자동 수정하지 않음

### 검증 규칙

- `defaultProject`가 `projects` 키에 반드시 존재해야 함
- alias는 소문자 영숫자 + 하이픈만 허용 (정규식: `^[a-z0-9]+(-[a-z0-9]+)*$`)
- 동일 `projectNumber`를 가진 alias가 중복되면 안 됨
- 동일 `projectUrl`을 가진 alias가 중복되면 안 됨
...
| `ownerType` | `X-GPM-Owner-Type` | 소유자 유형 |
| `repo` | `X-GPM-Repo` | 저장소 이름 |
| `projects[alias].projectNumber` | `X-GPM-Project-Number` | project resolve 키 |
| 선택된 alias | `X-GPM-Project-Alias` | 프로젝트 alias |
